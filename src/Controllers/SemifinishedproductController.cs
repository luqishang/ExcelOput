...
{
    public class SemifinishedproductController : Controller
    {
        ...
        /// コンストラクタ
        /// </summary>
        public SemifinishedproductController()
        {
        ...
        protected override void Initialize(System.Web.Routing.RequestContext requestContext)
        {
            ...
            string fileName = GetAppSet.GetAppSetValue("Screenexplanation", "Semifinishedproduct");
        ...
        public ActionResult Show()
        {
            ...
            var managementDt = from a in context.ManagementMs
                               ...
                               where a.SHOPID == shopId
                                  && a.MANAGEMENTID == ManagementID.HANSEIHIN
                               select a;
            ...
            CsvDef csvDefs = CsvManage.DEFS.Find(n => n.ManagementId == ManagementID.HANSEIHIN);
        ...
        /// 登録処理
        /// </summary>
        /// <param name="list">画面入力値(半製品リスト)</param>
        ...
        public ActionResult Edit(IList<BManagementM> list, List<int> posList, List<BUploadFile> uploadFileList)
        {
            ...
            {
                dt.ShopId = shopId;
                dt.ManagementId = ManagementID.HANSEIHIN;
                        ...
                        try
                        {
                            ...
                            foreach (ManagementM insModel in insManagementMs)
                            {
                                ...
                                insModel.MANAGEID = masterFunc.GetNumberingID(
                                    context: context, tableName: "MANAGEMENT_M", columnName: "MANAGEID", shopId: insModel.SHOPID, managementId: ManagementID.HANSEIHIN, digits: 5);
                                // データ登録
        ...
        public ActionResult FileUpload(IList<BManagementM> list, List<int> posList, List<BUploadFile> uploadFileList)
        {
            ...
            CsvDef csvDefs = CommonConstants.CsvManage.DEFS.Find(n => n.ManagementId == ManagementID.HANSEIHIN);
            ...
            foreach (var csvDt in listCsv)
            {
                ...
                {
                    hsError.Add("CSVファイルが不正です。半製品番号の値を入力してください。行番号=[" + lineNo + "] ");
                    checkError = false;
                }
                else if (targetManageNo.Length > 20)
                {
                    //桁数チェック
                    hsError.Add("CSVファイルが不正です。半製品番号は20文字以内で入力してください。行番号=[" + lineNo + "] ");
                    checkError = false;
                }
                //半製品名
                ...
                {
                    hsError.Add("CSVファイルが不正です。半製品名の値を入力してください。行番号=[" + lineNo + "] ");
                    checkError = false;
                }
                else if (targetManageName.Length > 20)
                {
                    // 桁数チェック
                    hsError.Add("CSVファイルが不正です。半製品名は20文字以内で入力してください。行番号=[" + lineNo + "] ");
                    checkError = false;
                }
                //単位
                ...
                {
                    hsError.Add("既存データが不正です。半製品番号（文字列可）=[" + targetManageNo + "] のデータが複数存在します。");
                    checkError = false;
                    continue;
                }
                ...
                else
                {
                    ...
                    model.SHOPID = shopId;
                    // 管理ID
                    model.MANAGEMENTID = ManagementID.HANSEIHIN;
                    // 表示No
                ...
                using (context = new MasterContext())
                {
                    using (var tran = context.Database.BeginTransaction())
                    {
                        try
                        {
                            ...
                            csvHistory.MANAGEMENTID = ManagementID.HANSEIHIN;
        ...
        private bool CheckRequire(BManagementM dt, ref HashSet<string> hsError)
        {
            ...
            {
                hsError.Add("半製品番号（文字列可）を入力してください。");
            ...
            {
                // 桁数チェック
                hsError.Add("半製品番号（文字列可）は20文字以内で入力してください。");
                ModelState.AddModelError("list[" + dt.No + "].ManageNo", string.Empty);
                checkError = false;
            }
            ...
            {
                // 桁数チェック
                hsError.Add("半製品名は20文字以内で入力してください。");
                ModelState.AddModelError("list[" + dt.No + "].ManageName", string.Empty);
                checkError = false;
            }
        ...
        private bool CheckUniqueness(BManagementM dt, List<BManagementM> updList, IQueryable<ManagementM> RegisteredList, ref HashSet<string> hsError)
        {
            ...
            if (!string.IsNullOrEmpty(dt.ManageNo))
            {
                ...
                {
                    hsError.Add("半製品番号（文字列可）が重複しています。No=[" + (dt.No + 1) + "]");
                    ModelState.AddModelError("list[" + dt.No + "].ManageNo", string.Empty);
                    checkError = false;
                }
                ...
                {
                    hsError.Add("半製品番号（文字列可）が既に登録されています。修正するか画面を更新して再度、登録してください。No=[" + (dt.No + 1) + "] 登録済半製品ID=[" + registDuplicateList[0].MANAGEID + "]");
                    ModelState.AddModelError("list[" + dt.No + "].ManageNo", string.Empty);
                    checkError = false;
                }
            }
