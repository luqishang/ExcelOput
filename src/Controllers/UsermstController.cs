...
namespace HACCPExtender.Controllers
{
    public class UsermstController : Controller
    {
        ...
        protected override void Initialize(System.Web.Routing.RequestContext requestContext)
        {
            ...
            string fileName = GetAppSet.GetAppSetValue("Screenexplanation", "Usermst");
        ...
        public ActionResult Show()
        {
            ...
            var managementDt = from a in context.ManagementMs
                             ...
                             where a.SHOPID == shopId
                                && a.MANAGEMENTID == ManagementID.USERMST
                             select a;
            ...
            CsvDef csvDefs = CsvManage.DEFS.Find(n => n.ManagementId == ManagementID.USERMST);
        ...
        /// 登録処理
        /// </summary>
        /// <param name="list">画面入力値(ユーザーマスタリスト)</param>
        ...
        public ActionResult Edit(IList<BManagementM> list, List<int> posList, List<BUploadFile> uploadFileList)
        {
            ...
            {
                dt.ShopId = shopId;
                dt.ManagementId = ManagementID.USERMST;
                        ...
                        try
                        {
                            ...
                            foreach (ManagementM insModel in insManagementMs)
                            {
                                ...
                                insModel.MANAGEID = masterFunc.GetNumberingID(
                                    context: context, tableName: "MANAGEMENT_M", columnName: "MANAGEID", shopId: insModel.SHOPID, managementId: ManagementID.USERMST, digits: 5);
                                // データ登録
        ...
        public ActionResult FileUpload(IList<BManagementM> list, List<int> posList, List<BUploadFile> uploadFileList)
        {
            ...
            // CSV行番号（Head行あり:1 ; Head行なし:0 。）
            int lineNo = 0;
            if (firstLineSkip)
            {
                lineNo = 1;
            }
            //CSVファイルのリストにして、内容をループする。
            foreach (var csvDt in listCsv)
            {
                ...
                //ユーザマスタ番号（文字列可）は空の文字列またはNULLをチェックする。（必須チェック）
                if (string.IsNullOrEmpty(targetManageNo))
                {
                    hsError.Add("CSVファイルが不正です。ユーザマスタ番号の値を入力してください。行番号=[" + lineNo + "] ");
                    checkError = false;
                }
                //ユーザマスタ番号（文字列可）の長さをチェックする。（桁数チェック）
                else if (targetManageNo.Length > 20)
                {
                    hsError.Add("CSVファイルが不正です。ユーザマスタ番号は20文字以内で入力してください。行番号=[" + lineNo + "] ");
                    checkError = false;
                }

                //ユーザマスタ名
                string targetManageName = csvDt[(csvColumns.Find(n => n.Name == "MANAGENAME").Pos - 1)];
                //ユーザマスタ名は空の文字列またはNULLをチェックする。（必須チェック）
                if (string.IsNullOrEmpty(targetManageName))
                {
                    hsError.Add("CSVファイルが不正です。ユーザマスタ名の値を入力してください。行番号=[" + lineNo + "] ");
                    checkError = false;
                }
                //ユーザマスタ名の長さをチェックする。（桁数チェック）
                else if (targetManageName.Length > 20)
                {
                    hsError.Add("CSVファイルが不正です。ユーザマスタ名は20文字以内で入力してください。行番号=[" + lineNo + "] ");
                    checkError = false;
                }
                ...
                {
                    hsError.Add("既存データが不正です。ユーザーマスタ番号（文字列可）=[" + targetManageNo + "] のデータが複数存在します。");
                    checkError = false;
                    continue;
                }
                ...
                else
                {
                    ...
                    model.SHOPID = shopId;
                    // 管理ID
                    model.MANAGEMENTID = ManagementID.USERMST;
                    // 表示No
                ...
                using (context = new MasterContext())
                {
                    using (var tran = context.Database.BeginTransaction())
                    {
                        try
                        {
                            ...
                            csvHistory.SHOPID = shopId;
                            csvHistory.MANAGEMENTID = ManagementID.USERMST;
                            csvHistory.UPLOADDATE = updateTime;
                            csvHistory.FILENAME = uploadFileList[0].UploadFile.FileName;
        ...
        [ValidateAntiForgeryToken]
        [HttpPost]
        public ActionResult CsvExport(IList<BManagementM> list)
        {
            // CSV管理情報を元に位置順に並び替えてリストを作成
            CsvDef csvDefs = CommonConstants.CsvManage.DEFS.Find(n => n.ManagementId == ManagementID.USERMST);
        ...
        private bool CheckUniqueness(BManagementM dt, List<BManagementM> updList, IQueryable<ManagementM> RegisteredList, ref HashSet<string> hsError)
        {
            ...
            if (!string.IsNullOrEmpty(dt.ManageNo)) {
                ...
                if (duplicateList.Count > 1) {
                    hsError.Add("ユーザーマスタ番号（文字列可）が重複しています。No=[" + (dt.No+1) + "]");
                    ModelState.AddModelError("list[" + dt.No + "].ManageNo", string.Empty);
                    checkError = false;
                }
                ...
                {
                    hsError.Add("ユーザーマスタ番号（文字列可）が既に登録されています。修正するか画面を更新して再度、登録してください。No=[" + (dt.No + 1) + "] 登録済ユーザーマスタID=[" + registDuplicateList[0].MANAGEID + "]");
                    ModelState.AddModelError("list[" + dt.No + "].ManageNo", string.Empty);
                    checkError = false;
                }
            }
