...
namespace HACCPExtender.Controllers
{
    public class SupplierController : Controller
    {
        ...
        protected override void Initialize(System.Web.Routing.RequestContext requestContext)
        {
            ...
            string fileName = GetAppSet.GetAppSetValue("Screenexplanation", "Supplier");
        ...
        public ActionResult Show()
        {
            ...
            var managementDt = from a in context.ManagementMs
                             ...
                             where a.SHOPID == shopId
                                && a.MANAGEMENTID == ManagementID.SHIIRESAKI
                             select a;
            ...
            CsvDef csvDefs = CsvManage.DEFS.Find(n => n.ManagementId == ManagementID.SHIIRESAKI);
        ...
        public ActionResult Edit(IList<BManagementM> list, List<int> posList, List<BUploadFile> uploadFileList)
        {
            ...
            {
                dt.ShopId = shopId;
                dt.ManagementId = ManagementID.SHIIRESAKI;
                        ...
                        try
                        {
                            ...
                            foreach (ManagementM insModel in insManagementMs)
                            {
                                ...
                                insModel.MANAGEID = masterFunc.GetNumberingID(
                                    context: context, tableName: "MANAGEMENT_M", columnName: "MANAGEID", shopId: insModel.SHOPID, managementId: ManagementID.SHIIRESAKI, digits: 5);
                                // データ登録
        ...
        public ActionResult FileUpload(IList<BManagementM> list, List<int> posList, List<BUploadFile> uploadFileList)
        {
            ...
            foreach (var csvDt in listCsv)
            {
                ...
                {
                    hsError.Add("CSVファイルが不正です。仕入先番号の値を入力してください。行番号=[" + lineNo + "] ");
                    checkError = false;
                }
                else if (targetManageNo.Length > 20)
                {
                    // 桁数チェック
                    hsError.Add("CSVファイルが不正です。仕入先番号は20文字以内で入力してください。行番号=[" + lineNo + "] ");
                    checkError = false;
                }
                //仕入先名
                ...
                {
                    hsError.Add("CSVファイルが不正です。仕入先名の値を入力してください。行番号=[" + lineNo + "] ");
                    checkError = false;
                }
                else if (targetManageName.Length > 20)
                {
                    // 桁数チェック
                    hsError.Add("CSVファイルが不正です。仕入先名は20文字以内で入力してください。行番号=[" + lineNo + "] ");
                    checkError = false;
                }
                ...
                } else
                {
                    ...
                    model.SHOPID = shopId;
                    // 管理ID
                    model.MANAGEMENTID = ManagementID.SHIIRESAKI;
                    // 表示No
                ...
                using (context = new MasterContext()) 
                {
                    using (var tran = context.Database.BeginTransaction())
                    {
                        try
                        {
                            ...
                            csvHistory.MANAGEMENTID = ManagementID.SHIIRESAKI;
        ...
        private bool CheckUniqueness(BManagementM dt, List<BManagementM> updList, IQueryable<ManagementM> RegisteredList, ref HashSet<string> hsError)
        {
            ...
            if (!string.IsNullOrEmpty(dt.ManageNo)) {
                ...
                if (duplicateList.Count > 1) {
                    hsError.Add("仕入先番号（文字列可）が重複しています。No=[" + (dt.No+1) + "]");
                    ModelState.AddModelError("list[" + dt.No + "].ManageNo", string.Empty);
                    checkError = false;
                }
                ...
                {
                    hsError.Add("仕入先番号（文字列可）が既に登録されています。修正するか画面を更新して再度、登録してください。No=[" + (dt.No + 1) + "] 登録済仕入先ID=[" + registDuplicateList[0].MANAGEID + "]");
                    ModelState.AddModelError("list[" + dt.No + "].ManageNo", string.Empty);
                    checkError = false;
                }
            }
